export default [
	{
		text: '精读书籍',
		link: '/books/Vue.js设计与实现/01_第 1、2、3 章框架设计概览',
	},
	{ text: '前端基础', link: '/fe_base/TypeScript/01_初识 TS 类型' },
	{ text: '前端框架', link: '/fe_framework/Vue/01-邂逅Vue3' },
	{ text: '后端基础', link: '/be_base/01_初识服务端' },
	{ text: '后端框架', link: '/be_framework/koa/01_Koa 基础' },
	{ text: '最佳实践', link: '/best_practice/JavaScript设计模式/01_面向对象和 UML 类图' },
	{ text: '算法', link: '/algorithm/01_初识数据结构与算法和栈' },
	{ text: '面试', link: '/interview/02-ES基础' },
]
