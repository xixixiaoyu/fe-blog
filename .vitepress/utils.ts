...
const sliceEnd = (item: string) =>
	item.indexOf(sortSep1) > -1 ? item.indexOf(sortSep1) : item.indexOf(sortSep2)
...
function getFile(pathname: string) {
	...
	const dir = fs
		...
		.sort((a, b) => {
			...
			if (a.startsWith('0')) {
				n1 = Num(a.replace('0', '').slice(0, sliceEnd(a)))
			}
			if (b.startsWith('0')) {
				n2 = Num(b.replace('0', '').slice(0, sliceEnd(b)))
			}
	...
	return dir.map(dir => {
		...
		const text = formatDir.slice(sliceEnd(formatDir) + 1)
		return {
			text,
